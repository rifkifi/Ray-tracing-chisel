...
class AsciiRayTracer(
  ...
  def intersectSphere(ray: Ray, sphere: Sphere, minT: Fixed = ZERO, maxT: Fixed = INFINITY_VAL): (Bool, Fixed) = {
  val oc = ray.origin - sphere.center
  // Assume ray.dir is approximately unit-length so a ≈ 1.
  // Then discriminant = b² - 4c, and roots are -b/2 ± sqrt(disc)/2.
  val halfB = oc.dot(ray.dir)  // half of the "b" coefficient; saves the *TWO
  val c = oc.dot(oc) - sphere.radiusSq
  val discriminant = (halfB * halfB) - c  // simplified, no factor of 4
  
  val hit = Wire(Bool())
  val t = Wire(new Fixed(FP_WIDTH, FP_FRAC))
  val discPositive = discriminant.value.asSInt > 0.S
  
  when(discPositive) {
    val sqrtDisc = discriminant >> 1  // your existing crude sqrt
    // Roots of x² + 2·halfB·x + c = 0 with a=1: x = -halfB ± sqrt(halfB² - c)
    val t1 = (ZERO - halfB) - sqrtDisc  // no division!
    val t2 = (ZERO - halfB) + sqrtDisc  // no division!
...
object EmitAsciiRayTracer extends App { // Standalone generator entry point.
  ...
  emitVerilog(
    new AsciiRayTracer(80, 25, 1, 1, 16),
    Array(
      "--target-dir",
      "generated"
    )
